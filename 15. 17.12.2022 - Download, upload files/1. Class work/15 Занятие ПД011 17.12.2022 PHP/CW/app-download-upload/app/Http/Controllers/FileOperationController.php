...
class FileOperationController extends Controller
{
    ...
    public function upload(Request $request) {
        ...
        // собственно оригинальное расширение
        // $extension = $file->getClientOriginalExtension();

        // $path = $file->storeAs('', $name);

        // сохранить в каталоге uploaded
        // $path = $file->storeAs('uploaded', $name);

        // генерация имени, определение расширения по MIME-типу
        // $name = $file->hashName();       // имя включает в себя расширение
        ...
        return view('file-operation.upload-result', ['path'=>$path]);
    } // upload
