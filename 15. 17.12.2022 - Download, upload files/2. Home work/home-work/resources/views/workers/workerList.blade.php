...
@section('content')
    ...
    <section class="mx-5 my-4 bg-light shadow-sm border rounded-3 min-vh-100 p-3">
        ...
        <div class="row">
            ...
            <div class="col-auto">
                <a class="btn btn-primary mt-2" href="/workers/downloadTempData">
                    <i class="bi bi-download"></i> Файл с выборкой
                </a>
            </div>
            @endif
        </div>
            ...
            <table class="table">
                ...
                @foreach($data as $item)
                    <tr class="align-middle {{isset($predicate) && $predicate($item) ? 'bg-light-green' : ''}}">
                        <th>{{$item->id}}</th>
                        <td>
                            <img src="{{asset("storage/people/$item->image")}}" class="h-5rem" alt="image">
                        </td>
                        <td>{{$item->fullName}}</td>
                        <td>{{$item->position}}</td>
