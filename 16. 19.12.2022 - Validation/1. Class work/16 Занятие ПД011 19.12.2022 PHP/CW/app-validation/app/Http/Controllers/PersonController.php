...
class PersonController extends Controller
{
    ...
    public function demo() {
        ...
        // если размер файла больше 300 байт - удалить файл
        if ($disk->exists($this->fileName_) && $disk->size($this->fileName_) > 500):
    ...
    public function handleAddPerson(Request $request) {
        ...
        // используем значение, возвращаемое validate() - это и есть данные формы
        // при ошибках валидации метод validate() переадресует на форму ввода
        // (повторный ввод)
        $validatedFields = $request->validate([
            'fullName' => 'bail|required|min:1',
            'age' => 'bail|required|integer|min:16|max:72',
            'salary' => 'bail|required|numeric|min:16000|max:45000000'
        ]);

        // заполнение полей объекта данными из формы
        $this->person_->fullName = $validatedFields['fullName'];
        $this->person_->age = $validatedFields['age'];
        $this->person_->salary = $validatedFields['salary'];
